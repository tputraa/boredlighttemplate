    ...
    <form action="{{ route('menus.update', $menu) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="icon" rows="3"
                class="mt-1 block w-full rounded-lg border border-slate-200 px-3 py-2 focus:border-[#800c3c] focus:ring-1 focus:ring-[#800c3c] outline-none font-mono text-xs">{{ old('icon', $menu->icon) }}</textarea>
        </div>
