                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-4 lg:px-6 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                @if ($userAccess['fedit'])
                                    <a href="{{ route('users.edit', $user) }}" class="text-slate-400 hover:text-blue-600 hover:bg-blue-50 p-1.5 rounded-md" title="Edit">
                                ...
                                @if ($userAccess['fdelete'])
                                    <form action="{{ route('users.destroy', $user) }}" method="POST" class="inline" onclick="return confirm('Are you sure?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-slate-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded-md" title="Delete">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
